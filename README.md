...
### 필수 종속성 설치 필요
```javascript
 $ yarn add react-native-orientation-locker react-native-svg react-native-track-player @react-native-community/netinfo react-native-safe-area-context react-native-webview react-native-localize  
```
### Pod Install (iOS)
```javascript
 $ npx pod-install ios
